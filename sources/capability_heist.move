...
module overmind::capability_heist {
    ...
    const ENTER_BANK_QUESTION: vector<u8> = b"What function is used to initialize a capability? Answer should start with lower-case letter";
    ...
    const TAKE_HOSTAGE_QUESTION: vector<u8> = b"Can you acquire a capability if the feature is not defined in the module you're calling from? (Yes/No)";
    ...
    const GET_KEYCARD_QUESTION: vector<u8> = b"How many ways are there to obtain a capability? Answer should contain only digits";
    ...
    const OPEN_VAULT_QUESTION: vector<u8> = b"Can capability be stored in the global storage? (Yes/No)";
